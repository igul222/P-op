...
class ConcatenatePop(Pop):
    def __init__(self, num_inputs, axis=1, **kwargs):
        super(ConcatenatePop, self).__init__(num_inputs, 1, **kwargs)
        self.num_inputs = num_inputs
        self.axis=axis # we default axis to 1 bc usually axis 0 is batchsize

    def symbolic_call(self, *args):
        if len(args) != self.num_inputs:
            raise ValueError("You called a ConcatentatePop with the wrong number of arguments. Expected: %s. got: %s" % (self.num_inputs, len(args)))

        return T.concatenate(list(args), axis=self.axis)
...
class ReshapePop(Pop):
    ...
    def __init__(self, shape, **kwargs):
        super(ReshapePop, self).__init__(1,1, **kwargs)
        shape = tuple(shape)
        for s in shape:
            if isinstance(s, int):
                if s == 0 or s < - 1:
    ...
    def symbolic_call(self, input, **kwargs):

...
def stretch_to_list(arg, listsize):
    if isinstance(arg, list):
        if len(arg) != listsize:
            raise ValueError ("tried to stretch %s to a list of size %s but it is a list of size %s" % (arg, listsize, len(arg)))
        else:
            return arg

    else:
        return [arg for i in range(listsize)]

def make_list(arg):
    """
    if arg is not a list, makes it a list
    """
    if isinstance(arg, list):
        return arg
